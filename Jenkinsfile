pipeline {
    ...
    stages {
        ...
        stage ('Var') {
            steps {
                script {
                    azureCLI commands: [[exportVariablesString: '', script: 'StagingPublicIP=$(az vm show -d -g nginx-server-fabio-staging -n nginx-webserver -u fabio --query publicIps -o tsv)']], principalCredentialId: ''
                }
            }
        }
